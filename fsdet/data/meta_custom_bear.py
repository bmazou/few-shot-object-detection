...
def custom_bear_loader(name, load_classes):
    if name == 'custom_bear_all':
        shots = 10
    else:
        shots = float('inf')
    data = []
    ...
    for obj in classes:
        data += get_annotations_of_object('datasets/custom_bear/', obj, thing_classes, shots)
...
class CustomBearEvaluator(DatasetEvaluator):
...
def register_custom_bear(name, thing_classes, metadata):
    # register dataset (step 1)
    DatasetCatalog.register(
        name, # name of dataset, this will be used in the config file
        lambda: custom_bear_loader( # this calls your dataset loader to get the data
    ...
    MetadataCatalog.get(name).evaluator_type = "custom_bear" # set evaluator
